...
TimeSeries::TimeSeries() : seriesName(""), seriesCode(""), years(new int[2]), data(new double[2]), current(0), capacity(2), next(nullptr){
...
//new load function called in project 2
void TimeSeries::loadSeries(int year, double num, int index) {
    if(current >= capacity) resize(true);
    years[index] = year; 
    data[index] = num;
    current++;
}
...
bool TimeSeries::update(int year, double value){
    ...
    //handle deletion v1
    //solved (unsolved issue: some data are assigned to xxxxe-311, very weird)
...
//enables main to get the data size
int TimeSeries::getCurrent() const {
    return current;
}

//enables SeriesList to get series name and code
void TimeSeries::setSeriesName(std::string value){
    seriesName = value;
}

std::string TimeSeries::getSeriesName() const{
    return seriesName;
}

void TimeSeries::setSeriesCode(std::string value){
    seriesCode = value;
}

std::string TimeSeries::getSeriesCode() const{
    return seriesCode;
}

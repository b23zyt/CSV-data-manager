...
void Hashing::printLess(double value, TreeNode* node) {
    if(node == nullptr){
        return;
    }
...
void Hashing::printGreat(double value, TreeNode *node){
    if(node == nullptr){
        return;
    }
...
void Hashing::printEqual(double value, TreeNode *node){
    double tolerance = 1e-3;
    //not needed actually
    if (node == nullptr){
        return;
...
void Hashing::find(double mean, std::string operation){

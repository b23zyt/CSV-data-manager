...
SeriesList::SeriesList() : countryName(""), countryCode(""), head(nullptr) {
}
...
bool SeriesList::load(std::string country) {
    ...
    while(std::getline(myFile, line)) {
        ...
        /*
        Adjustments
        I Initialized countryNameTemp countryCodeTemp after failing test 2 and test 3
        Previous code: std::getline(lineStream, countryName , ','); std::getline(lineStream, countryCode , ',');

        The issue was that the while loop iterates through all lines in the dataset, causing countryName and countryCode to store the country name/code of the last series in the dataset. However, I needed them to store the country name/code of the matching series.

        To fix this, I introduced 2 new variables countryNameTemp and countryCodeTemp. I updated them with the series's name and code in every loop. When a match is found between countryNameTemp and the input, I assign these temp variables to the class's private members (countryName and countryCode). 
        */
        ...
        /* 
        Begin to Load data
        Mostly copy from Project1 load function
        */
        flag = true;
        ...
        while(std::getline(lineStream, dataInString, ',')){
            ...
            /*
            //version 1 (requires too much of time although it works. O(n^2) too slow...)
            bool dummy = newSeries->add(year, dataInDouble);
            */
